...
def test_v092_version_release_docs_and_package_defaults():
    ...
    assert (ROOT / "docs" / "V1_0_RELEASE_PRECHECK_v0.9.2.md").is_file()
    assert (ROOT / "docs" / "FINAL_REGRESSION_REPORT_v0.9.2.md").is_file()
...
def test_v092_pdf_minimal_flow_and_export_state_are_stable():
    with TestClient(app) as client:
        project_id = create_project(client, "v0.9.2 PDF 最终回归")
...
def test_v092_dxf_minimal_flow_cad_json_candidates_and_excel_are_stable():
    with TestClient(app) as client:
        project_id = create_project(client, "v0.9.2 DXF 最终回归")
...
def test_v092_dwg_mock_conversion_flow_does_not_directly_parse_dwg(tmp_path: Path):
    clear_converter_tables()
    converter = write_mock_converter(tmp_path)
    with TestClient(app) as client:
        project_id = create_project(client, "v0.9.2 DWG 最终回归")
...
def test_v092_cad_pipeline_review_workbench_and_manual_field_protection(tmp_path: Path):
    clear_converter_tables()
    converter = write_mock_converter(tmp_path)
    with TestClient(app) as client:
        create_converter_setting(client, converter)
        project_id = create_project(client, "v0.9.2 pipeline 最终回归")
        upload = upload_files(
            client,
            project_id,
            [
        ...
        blocked_confirm = client.post(
            ...
            json={"force": False, "note": "v0.9.2 open error 拦截"},
        )
        confirm = client.post(f"/api/sheets/{dxf_sheet_id}/confirm", json={"force": True, "note": "v0.9.2 确认"})
...
def test_v092_export_does_not_change_review_or_issue_status():
    with TestClient(app) as client:
        project_id = create_project(client, "v0.9.2 导出一致性")
...
def test_v092_backup_restore_verify_delete_and_original_project_are_safe():
    with TestClient(app) as client:
        project_id = create_project(client, "v0.9.2 备份恢复最终回归")
...
def test_v092_cad_preview_skip_force_and_failed_preview_do_not_block_export():
    with TestClient(app) as client:
        project_id = create_project(client, "v0.9.2 CAD 预览最终回归")
...
def test_v092_data_health_and_temp_cleanup_are_diagnostic_and_safe():
    with TestClient(app) as client:
        project_id = create_project(client, "v0.9.2 健康检查最终回归")
...
def test_v092_portable_package_contains_required_release_materials_and_clean_app_data():
