...
const APP_VERSION = "v0.6.2-rule-fix";
const APP_TITLE = `工程图纸智能台账识别系统 ${APP_VERSION}`;
...
function isCadReadyFile(file: DrawingFile) {
...
function pipelineStepLabel(step: CadPipelineStep) {
...
function pipelineSuccessTotal(result: CadPipelineResponse) {
  return result.steps.reduce((total, step) => total + step.success_count, 0);
}

function pipelineFailureTotal(result: CadPipelineResponse) {
  return result.steps.reduce((total, step) => total + step.failed_count, 0);
}

function pipelineNextSuggestion(result: CadPipelineResponse) {
...
function pipelineErrorSuggestion(errorCode: string) {
  const labels: Record<string, string> = {
...
function recommendedCandidate(fieldName: string, candidates: RecognitionCandidate[]) {
  return candidates
    .filter((candidate) => candidate.field_name === fieldName)
    .sort((left, right) => right.confidence - left.confidence || left.id - right.id)[0];
}

function fieldSourceDescription(value: FieldValue | undefined, evidence: FieldEvidence[]) {
  if (!value) {
    return "暂无推荐字段";
  }
...
function errorCodeMessage(errorCode: string, fallback: string) {
  const labels: Record<string, string> = {
...
function fieldNameLabel(fieldName: string) {
  const labels: Record<string, string> = {
    drawing_no: "图纸编号",
    drawing_name: "图纸名称",
    discipline: "专业",
...
function singleRecognitionAsBatch(
...
function CandidateGroups({ candidates }: { candidates: RecognitionCandidate[] }) {
